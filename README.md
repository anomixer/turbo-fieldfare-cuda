...
## Performance reality check (read before choosing this runtime)

This fork is not currently a CUDA throughput winner. In the same-prompt-text
benchmarks, llama.cpp generated faster than TurboFieldfare on every tested
NVIDIA discrete GPU:

| GPU | TurboFieldfare | llama.cpp | llama.cpp advantage |
| --- | ---: | ---: | ---: |
| NVIDIA L4 | 15.2–28.5 tok/s | 20.5–77.2 tok/s | about 1.3–3.1x |
| RTX 4000 SFF Ada | 13.4–26.4 tok/s | 16.8–75.5 tok/s | about 1.3–3.7x |
| T1000 8GB | 6.0–6.7 tok/s | 16.6–17.0 tok/s | about 2.5–2.8x |

The measurements use the same prompt text, but the runtimes tokenize it
differently (`.gturbo` reports 91 tokens and GGUF reports 83). See the
hardware-specific benchmark documents for raw results and methodology.

TurboFieldfare's more plausible fit is a UMA/shared-memory system—such as
Apple Silicon, an AMD APU, or an Intel integrated GPU—where CPU and GPU share
memory and SSD/expert streaming may be less punitive. Those platforms are not
supported by this Windows CUDA fork today: Mac requires a Metal backend, while
AMD/Intel require a compatible HIP, Vulkan, or D3D12 backend. Treat this as a
design direction, not a compatibility claim.

Choose this fork for `.gturbo` Gemma 4 support, controllable VRAM budgets,
expert streaming, and the local server/GUI integration. If your priority is
raw NVIDIA CUDA tokens per second, use llama.cpp instead.
...
```text
Download release
  -> build (source checkout only; prebuilt users can skip this)
  -> dlmodel
  -> quickbench
  -> server
...
The Turing-class low-VRAM result is documented in
[`docs/T1000_8GB_BENCHMARK.md`](docs/T1000_8GB_BENCHMARK.md).
The companion llama.cpp run is in
[`docs/T1000_8GB_LLAMA_BENCHMARK.md`](docs/T1000_8GB_LLAMA_BENCHMARK.md).
The RTX 4000 SFF Ada llama.cpp baseline is in
[`docs/RTX4000_SFF_ADA_LLAMA_BENCHMARK.md`](docs/RTX4000_SFF_ADA_LLAMA_BENCHMARK.md).
The L4 llama.cpp benchmark using the unified prompt is in
[`docs/L4_LLAMA_BENCHMARK.md`](docs/L4_LLAMA_BENCHMARK.md).
