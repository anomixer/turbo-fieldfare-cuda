...
rem WDDM reports desktop composition processes (for example SearchApp.exe)
rem under --query-compute-apps even when they use only a few MiB.  Use the
rem aggregate GPU allocation instead, and ignore the normal desktop baseline.
set "GPU_USED="
for /f "tokens=1" %%U in ('nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits 2^>nul') do if not defined GPU_USED set "GPU_USED=%%U"
if not defined GPU_USED (
    echo ERROR: Could not query current GPU memory usage with nvidia-smi.
    exit /b 2
)
set /a GPU_USED_NUM=GPU_USED+0
if %GPU_USED_NUM% GTR 512 (
    echo.
    echo WARNING: GPU memory currently in use: %GPU_USED_NUM% MiB.
    echo ERROR: Stop applications using substantial GPU memory before running quickbench.
